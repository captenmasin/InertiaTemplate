...
exports.createCalculateSortIndex = exports.createCalculateSpecifierSortIndex = exports.getFirstNotSorted = exports.getImportType = exports.getImportsWithNodesBetween = exports.isImportDeclaration = void 0;
...
function getFirstNotSorted(imports, calculateSortIndex) {
    const isNodesSorted = (a, b) => calculateSortIndex(a) <= calculateSortIndex(b);
...
function getSpecifiersLength(node, sourceCode) {
    const commaLength = 1;
    const specifiersLength = node.specifiers.reduce((length, node) => getNodeLength(node, sourceCode) + commaLength + length, 0);
    return specifiersLength - commaLength;
}
const createCalculateSpecifierSortIndex = (sourceCode, options) => (node) => node && !options.disableLineSorts
